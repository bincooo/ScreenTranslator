...
test('renderer HTTP bridge blocks local-network abuse and disables redirects', () => {
  ...
  assert.match(http.text, /redirect: 'manual'/)
  ...
  assert.match(functionText(http, 'isConfiguredPrivateProviderTarget'), /isLocalOrPrivateHost/)
...
test('plugin runtime sandbox exposes only explicit utils RPC and checks message origin by frame', () => {
  ...
  assert.match(functionText(pluginRuntime, 'getPluginRuntime'), /baseDir: FileBase\.Config/)
})
